...
/**
 * Creates or overwrites a monthly pass document in the database
 ...
 */
async function getMonthlyPass(userId, passId) {
  try {
    const docRef = doc(db, "users", userId, "monthlyPasses", passId);
...
/**
 * Reads all monthly pass documents for a user
 * @param {string} userId - The user ID
 * @returns {Promise<Array>} Array of monthly pass objects
 */
async function getAllMonthlyPasses(userId) {
  try {
    const querySnapshot = await getDocs(collection(db, "users", userId, "monthlyPasses"));
    const monthlyPasses = [];
 ...
 * @returns {Promise<string>} The pass ID
 */
async function updateMembership(userId, passId, updates) {
  try {
    const docRef = doc(db, "users", userId, "monthlyPasses", passId);
...
/**
 * Cancels a monthly pass by setting cancelled_date
 * @param {string} userId - The user ID
 * @param {string} passId - The monthly pass ID
 * @returns {Promise<string>} The pass ID
 */
async function cancelMonthlyPass(userId, passId) {
  try {
    const docRef = doc(db, "users", userId, "monthlyPasses", passId);
